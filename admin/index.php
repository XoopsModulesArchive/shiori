...
switch ($op) {
    ...
    case 'lank':
        ...
        $days = (isset($_GET['days'])) ? (float)$_GET['days'] : 7;
        $secago = time() - $days * 24 * 60 * 60;

        //URL
        $criteria = ['date>' . $secago];
        ...
        foreach ($counturls_arr as $url => $sum) {
            ...
            echo "\n" . '<td><a href="' . htmlspecialchars($url, ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($url, ENT_QUOTES | ENT_HTML5) . '</a></td>';
        ...
        foreach ($mods_arr as $mid => $sum) {
            ...
            echo "\n" . '<td>' . htmlspecialchars($modname, ENT_QUOTES | ENT_HTML5) . '</td>';
