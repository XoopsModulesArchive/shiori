...
if ('form' == $op) {
    ...
    if (!preg_match('|' . XOOPS_URL . '|', $_REQUEST['url']) && 0 == $xoopsModuleConfig['shiori_prmt_outofsite']) {
    ...
    $url = $myts->stripSlashesGPC(htmlspecialchars($_REQUEST['url'], ENT_QUOTES | ENT_HTML5));
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'shiori_form.html';
...
if ('save' == $op) {
    ...
    if (!preg_match('|' . XOOPS_URL . '|', $_POST['url']) && 0 == $xoopsModuleConfig['shiori_prmt_outofsite']) {
        redirect_header(XOOPS_URL, 3, _MD_NOT_THISITE);
    ...
    $nottrans = isset($_POST['nottrans']) ? (int)$_POST['nottrans'] : 0;
